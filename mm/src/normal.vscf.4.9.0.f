...
*               if NROTTR=0, will return all 3*NATOM-6 normal modes
*               if NROTTR<-1, will return 3*NATOM-6+NROTTR highest-freq modes
...
*            ordering in omega.
*       ASSUMING the molecule is non-linear and at a minimum
*************************************************************************
...
*  NROTTR can be a negative number
*************************************************************************
          nmodes=3*natom
          n6=nmodes-nmode  ! nmode=nmodes-6+nrottr, so n6=6-nrottr
          do i=1,natom
           do j=1,3
             xlat(i,j)=x0(i,j)
           end do
...
CCCC      delta = 0.05d0
          delta = 0.100d0
CCCCCC    delta = 0.500d0
CCCCCC    delta = 1.000d0
        do i = 1,nmodes
          coord(i) = coord0(i)
        end do
        ntmd = 0
...
*************************************************************************
        ...
        imm = 1
        do mm = 1,nmodes,3
          xlat(imm,1) = coord0(mm)*bhr/cmass(imm)
          xlat(imm,2) = coord0(mm+1)*bhr/cmass(imm)
          xlat(imm,3) = coord0(mm+2)*bhr/cmass(imm)
          imm = imm+1
        end do
...
C       VMIN=POTENS(0,0)
        WRITE(IOUT,*)
        WRITE(IOUT,*)'V AT EQU ',POTENS(0,0)*WAVENM,'CM-1'
        WRITE(IOUT,*)
        CALL FLUSH(IOUT)
          do i = 1,nmodes
            do j = i,nmodes
...
*************************************************************************
                    ...
                    imm = 1
                    do mm = 1,nmodes,3
                      xlat(imm,1) = coord(mm)*bhr/cmass(imm)
                      xlat(imm,2) = coord(mm+1)*bhr/cmass(imm)
                      xlat(imm,3) = coord(mm+2)*bhr/cmass(imm)
                      imm = imm+1
                    end do
...
c  check for imaginary frequencies (<-10cm-1)
        ...
        IF(FREQ(I)*WAVENM.LT.-50)THEN
          write(2,*) "ERROR: HAS LARGE IMAGINARY FREQUENCY"
          write(2,*) "MAYBE AT A SADDLE POINT OR BAD MINIMUM"
          stop
        ENDIF !INEGS=INEGS+1
      end do
...
c  stuff rnorms into xl
        do n=1,nmode
          ...
          enddo
        enddo
C     WRITE(IOUT,*)'VECTORS'
C     DO N=1,NMODES
C       IC=0
C       DO M=1,NATOM
C       WRITE(IOUT,*)RNORMS(IC+1,N),RNORMS(IC+2,N),RNORMS(IC+3,N)
C       IC=IC+3
C       END DO
C     END DO
      ELSE
